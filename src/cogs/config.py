...
class Config(commands.Cog):
    ...
    async def set_timezone(self, interaction: discord.Interaction):
        if AccountDAO.getAccountById(interaction.user.id):
            await interaction.response.send_message(embed=self.embeds_set_timezone["url"],view=SetzView() , ephemeral=True)
        else:
            await interaction.response.send_message(embed=self.embeds_set_timezone["error"], ephemeral=True)
